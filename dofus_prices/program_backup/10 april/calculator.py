...
def load_rune_prices():
    ...
    for rune in rune_prices_temp:
        ...
        if rune_name in rune_names:
            if rune_name == "pods" or rune_name == "initiative":
                rune_prices[rune_names[rune_name]] = rune[1]/10
            elif rune_name == "vitality":
...
def load_ingredient_data():
    ...
    ingredient_price_log.insert(tk.INSERT, "Ingredients loaded!") 
...
def load_recipe_data():
    ...
    for recipe in recipes:
        ...
        for stat in recipe.stats:
            ...
            regular_profits += float(stat[0])*rune_prices[stat[1]]
        ...
        for stat in recipe.stats:
            ...
            profit = (focus_sink/rune_sink[stat[1]])*rune_prices[stat[1]]
            if profit > best_profits[1]:
                best_profits = [stat[1], profit]
        profit_percentage = best_profits[1]/total_cost
        item_profits.append([recipe.name, best_profits[0], profit_percentage])
    item_profits.sort(key=lambda x: x[2], reverse=True)
