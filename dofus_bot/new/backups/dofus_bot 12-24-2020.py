...
def calibrate():
    global status, game_region, current_coordinates, current_path, market_pos, resources_of_interest, price_percentage
    reset_variables()
    try:
...
def sell_item(item_type):
    ...
    if item is not None:
        ...
        try:
            resource_amount = pytesseract.image_to_string(resource_amount_filtered, config="--psm 7")
...
def sell_on_market():
    ...
    if market_text is None:
        ...
        if not resources_to_check:
            reset_variables(reset_status=True)
