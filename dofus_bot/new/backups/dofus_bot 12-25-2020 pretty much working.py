...
def calibrate():
    global game_region, current_coordinates, current_path, market_pos, resources_of_interest, price_percentage
    reset_variables()
    try:
            ...
            try:
                ...
                try:
                    ...
                    for coordinate in path_coordinates:
                        ...
                        current_path.append([int(coordinate_split[0]), int(coordinate_split[1])])
                    if current_path:
                        try:
    ...
    try:
        ...
        if not resource_list:
            ...
            if len(sorted_pos) > 0:
                ...
                resource_list = [resource_list_filtered.pop(0)]
...
def harvest_resources():
    ...
    if current_coordinates != current_path[0]:
        move_to(current_path[0])
    else:
        ...
        if not resources_to_check:
            ...
            new_coordinate = current_path.pop(0)
...
def sell_resources():
    global resources_to_check, status
    if current_coordinates != market_pos:
        move_to(market_pos)
    else:
        market_text = pyautogui.locateCenterOnScreen('market_text.png')
        if market_text is None:
            # If the market hasn't been opened yet
            # Takes a screenshot with and without highlight
            screenshot = pyautogui.screenshot(region=game_region)
            pyautogui.keyDown('y')
            screenshot_highlight = pyautogui.screenshot(region=game_region)
...
def exit_map(direction):
    # TODO check if exiting in a direction is possible
    if direction == 'north':
...
def move_to(goal=None):
    global current_coordinates, coordinates_goal, status
    ...
    try:
        ...
        if status[-1] == 'moving':
            if detected_coordinates != current_coordinates:
                ...
                if current_coordinates == coordinates_goal:
                    status.pop(-1)
                    log("Finished moving.")
                else:
                    navigate(coordinates_goal)
        else:
...
def reset_variables(reset_status=True):
    global status, resource_list, clicked_resource, last_known_price
    if reset_status:
        status = []
    resource_list = []
...
path_text = tk.Label(gui, text="Current path: [x,y][x,y][x,y]")
