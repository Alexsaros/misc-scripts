...
def harvest_resource_on_screen(resource_type):
    ...
    try:
        ...
        if not resource_list:
            ...
            cv2.imwrite('2.png', img_filtered)
            cv2.imwrite('3.png', img_highlight_filtered)
            # Takes only the white that appeared when holding the y button
            highlights_filtered = img_highlight_filtered - img_filtered
            cv2.imwrite('4.png', highlights_filtered)
            # Fill in the outlines of the interactable objects
            kernel = np.ones((int(h/2), int(w/2)), np.uint8)
            highlights_filled = cv2.morphologyEx(highlights_filtered, cv2.MORPH_CLOSE, kernel)
            # Only look at what's inside the outlines in the color image
            img = cv2.cvtColor(np.array(screenshot), cv2.COLOR_RGB2BGR)
            cv2.imwrite('1.png', img)
            filtered = cv2.bitwise_and(img, img, mask=highlights_filled)
            cv2.imwrite('gray.png', filtered)
            # TODO use gray filtered images
