...
async def handle_command():
    ...
    if command[:4] == 'help':
        ...
        command = ''
        await map()
    elif command == 'stats' or command == 'statistics' or command == 'stat':
        command = ''
        await stats()
...
async def playerlist():
    message = ''
    for uniquePlayer in playerList:
        message += f"{uniquePlayer.name} | {uniquePlayer.character_class} | {uniquePlayer.race} | level {uniquePlayer.level}\n"
...
async def stats():
    await channel.send(f'**{player.name}**\nLevel {player.level} {player.race} {player.character_class}'
                       f'\nMax HP: {player.max_hp} \t Base armor: {player.armor}'
...
async def race_selection():
    ...
    elif command == 'human' or command == 'dwarf' or command == 'elf':
        player.race = command
        command = ''
        await race_selection_confirmation()
    else:
...
async def battle_confirmation():
    ...
    if command == 'y' or command == 'yes':
        ...
        command = ''
        player.enemies_turn = False
        player.enemy.enemies_turn = True
        await battle()
...
async def battle():
    ...
    if command == '':
        ...
        enemies_turn = await chosen_action(command, player, player.enemy)
    else:
        try:
            command = int(command)
            ...
            if not action_in_hand:
                await channel.send("This action is invalid.")
    if enemies_turn and player.enemy.current_hp > 0:
        try:
            if player.enemy.discordName != '':
                print('you\'re fighting a human')
            ...
            if player.enemy.distance > 1:
                enemies_turn = await chosen_action('move', player.enemy, player)
                await channel.send(f'\nThe {player.enemy.name} approaches you.')
            else:
    ...
    if player.enemy.current_hp <= 0:
        ...
        for action in player.actions_hand:
            index += 1
            available_actions += f'\n {index}. {action}'
        await channel.send(f'The {player.enemy.name} is {round(player.enemy.distance, 2)} meters away and has {player.enemy.current_hp}/{player.enemy.max_hp} HP and {player.enemy.armor+player.enemy.armor_bonus} armor.'
...
async def new_action():
    if len(player.actions_pool) == 0:
        player.actions_pool = player.actions.copy()
    new_action_index = random.randint(0, len(player.actions_pool)-1)
    new_action = player.actions_pool[new_action_index]
    player.actions_hand.append(new_action)
    player.actions_pool.remove(new_action)
