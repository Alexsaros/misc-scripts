...
def show_screen():
    ...
    offset = 0
    for line in log:
        if line_number+offset+int((len(line) - 2) / MAIN_WINDOW_WIDTH)+1 < LOG_LENGTH:   # checks if still room on log
    ...
    if game_started:
        map_full = return_map()
        map_full = np.flip(map_full, 1)  # mirrors y so y+ is up
        for x in range(map_width):
            for y in range(map_height - 1, -1, -1):
                map_window.addstr(y, (x * 3) + 1, map_full[x, y])
    screen.refresh()
    if game_started:
        if game_mode == 'p':
        ...
        elif position[1] < MAIN_WINDOW_HEIGHT/2:    # south border lock
    ...
    while len(open_list) > 0:
        ...
        for neighbour in neighbours:  # selects tiles next to current tile
            ...
            if 0 <= new_tile[0] < map_width and 0 <= new_tile[1] < map_height:
                ...
                if (map_creatures[new_tile[0], new_tile[1]] == '' or new_tile[:2] == goal) and map_terrain[
                    new_tile[0], new_tile[1]].blocks_movement == False:
                    ...
                    new_tile[2] = (
                                current_tile[2] + 1 - get_distance(current_tile, goal) + get_distance(new_tile, goal))
    ...
    try:
        if 1 <= int(command) <= 9 and int(command) != 5:
            ...
            if 0 <= new_pos[0] < map_width and 0 <= new_pos[1] < map_height:
                if map_creatures[new_pos[0], new_pos[1]] == '' and map_terrain[
                    new_pos[0], new_pos[1]].blocks_movement == False:
                    player.x = new_pos[0]
                    player.y = new_pos[1]
                    end_turn = True
                # moving onto occupied tile attacks creature if melee weapon
                elif map_creatures[
                    new_pos[0], new_pos[1]] != '' and player.weapon.min_range <= 1 and player.weapon.max_range <= 1:
                    for creature in creature_list:
...
at_start_of_game()
map_window = curses.newpad(map_height, map_width * 3)
...
# improve spawning rates, improve vision/ranged attacks, fix watch mode scrolling view
while not quit_game:
    game_started = True
    if game_mode == 'w':
        cursor_mode = True
