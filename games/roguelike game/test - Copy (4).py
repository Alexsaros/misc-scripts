...
def init_map_terrain(empty=True):
    map_empty = init_map_empty()
    for x in range(map_width):
        for y in range(map_height):
            map_empty[x, y] = EmptyTile()
    if not empty:
...
def show_map():
    ...
    for y in range(map_height):
        for x in range(map_width):
            ...
            elif map_terrain[x, y].symbol != '':       # bottom layer is terrain
...
def target_in_sight(creature, target):
    print()
...
def basic_turn(creature):
    ...
    if creature.alignment == "evil":
        ...
        for enemy in creature_list:     # attacks all evil characters teams
...
npc_spawn_directions = ["yyyy", "yyyy", "nnnn"]
...
log = []
quit_game = False
# todo add spells, wall block arrows, with map editor edit creature type
# improve spawning rates, add colours, sight
while not quit_game:
    if game_mode == 'w':
        show_only_attacks_on_player = False
        player = None
